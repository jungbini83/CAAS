...
if [ $netType == "grid" ]; then
	...
	routecmd="python /usr/local/share/sumo/tools/randomTrips.py -n ./map/${netType}/${netType}.net.xml -e 180 -r ./map/${netType}/${netType}.rou.xml --trip-attributes=departLane=\"best\" departSpeed=\"random\" departPos=\"random\" --intermediate 10 -p 7 -b 0.1 -o ./map/${netType}/${netType}.trips.xml"		
elif [ $netType == "spider" ]; then
	netcmd="netgenerate --spider --arms=5 --circles=3 --radius=100 --output-file=./map/${netType}/${netType}.net.xml"
	routecmd="python /usr/local/share/sumo/tools/randomTrips.py -n ./map/${netType}/${netType}.net.xml -e 180 -r ./map/${netType}/${netType}.rou.xml --trip-attributes=departLane=\"best\" departSpeed=\"random\" departPos=\"random\" --intermediate 10 -p 1 -o ./map/${netType}/${netType}.trips.xml"		
